...
public class TitanAI : EnemyBase, IBegin
{
    ...
    void Start()
    {
        ...
        if (spriteTransform == null) Debug.LogError("Sprite Transform 변수가 비어있습니다! 자식 스프라이트/모델 오브젝트를 할당해주세요.", gameObject);
        if (fov == null) Debug.LogError("FOVController(fov) 컴포넌트를 찾을 수 없습니다! EnemyBase 또는 TitanAI를 확인해주세요.", gameObject);
        if (mastTarget == null) Debug.LogError("mastTarget(Engine 태그)을 찾을 수 없습니다!", gameObject);
    ...
    private void UpdateTarget()
    {
        ...
        // 시야에 보이는 모든 타겟 중 가장 가까운 '장애물'을 찾음
